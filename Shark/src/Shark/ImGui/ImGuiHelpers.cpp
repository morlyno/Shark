...
bool ImGui::TableNextColumn(ImGuiTableRowFlags row_flags = 0, float min_row_height = 0.0f)
{
	...
	if (table->IsInsideRow && table->CurrentColumn + 1 < table->ColumnsCount)
	{
		if (table->CurrentColumn != -1)
			ImGui::TableEndCell(table);
		ImGui::TableBeginCell(table, table->CurrentColumn + 1);
	}
	else
	{
		ImGui::TableNextRow(row_flags, min_row_height);
		ImGui::TableBeginCell(table, 0);
	}
...
namespace ImGui {
	...
	bool InputTextEx(ImGuiID id, const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
	{
		ImGuiWindow* window = GetCurrentWindow();
		if (window->SkipItems)
			return false;
