...
namespace Shark {
	...
	Ref<Texture2D> Texture2D::Create(uint32_t width, uint32_t height, uint32_t color, const std::string& name)
	{
		switch (RendererAPI::GetAPI())
		{
			case RendererAPI::API::None: SK_CORE_ASSERT(false, "RendererAPI not specified"); return nullptr;
			case RendererAPI::API::DirectX11: return CreateRef<DirectXTexture2D>(width, height, color, name);
		}
