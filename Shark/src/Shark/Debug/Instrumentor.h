	...
	public:
		...
		void BeginSession(const std::string& name, const std::string& filepath = "results.json")
		{
			...
			if (m_CurrentSession)
			{
				...
				{
					SK_CORE_ERROR("Instrumentor::BeginSession('{0}') when session '{1}' already open.", name, m_CurrentSession->Name);
				}
				InternalEndSession();
			}
			...
			else
			{
				if (Log::GetCoreLogger()) // Edge case: BeginSession() might be before Log::Init()
				{
					SK_CORE_ERROR("Instrumentor could not open results file '{0}'.", filepath);
				}
			}
		}
...
#define SK_PROFILE_BEGIN_SESSION(name, filepath) ::Shark::Debug::Instrumentor::Get().BeginSession(name, filepath)
#define SK_PROFILE_END_SESSION() ::Shark::Debug::Instrumentor::Get().EndSession()
#define SK_PROFILE_SCOPE_LINE2(name, line) constexpr auto fixedName##line = ::Shark::Debug::InstrumentorUtils::CleanupOutputString(name, "__cdecl ");\
											   ::Shark::Debug::InstrumentationTimer timer##line(fixedName##line.Data)
