...
void main(uint3 dispatchID : SV_DispatchThreadID)
{
    ...
    float cosLo = dispatchID.x / float(TextureSize(o_LUT).x);
    float roughness = dispatchID.y / float(TextureSize(o_LUT).y);
    ...
    o_LUT[dispatchID.xy] = float4(DFG1, DFG2, 0, 0) * InvNumSamples;
}