...
VertexToPixel main(VertexInput Input)
{
    ...
    matrix boneTransform = mul(u_BoneTransforms[u_MeshData.BoneBase + Input.BoneIDs.x].Transform, Input.BoneWeights.x);
    boneTransform       += mul(u_BoneTransforms[u_MeshData.BoneBase + Input.BoneIDs.y].Transform, Input.BoneWeights.y);
    boneTransform       += mul(u_BoneTransforms[u_MeshData.BoneBase + Input.BoneIDs.z].Transform, Input.BoneWeights.z);
    boneTransform       += mul(u_BoneTransforms[u_MeshData.BoneBase + Input.BoneIDs.w].Transform, Input.BoneWeights.w);
    
    matrix transform = mul(u_MeshData.Transform, boneTransform);
    float3x3 objectTransform = mul((float3x3)u_MeshData.Transform, (float3x3)boneTransform);
    
    float4 pos = mul(transform, float4(Input.Position, 1.0f));
    Output.Position = mul(u_Camera.ViewProjection, pos);

    
    Output.WorldPosition = pos.xyz;
    Output.ViewPosition = u_Camera.Position;
    Output.Normal          = normalize(mul(objectTransform, Input.Normal));
    Output.WorldNormals[0] = normalize(mul(objectTransform, Input.Tangent));
    Output.WorldNormals[1] = normalize(mul(objectTransform, Input.Bitangent));
    Output.WorldNormals[2] = normalize(mul(objectTransform, Input.Normal));
