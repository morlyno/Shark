...
void main(uint2 dispatchID : SV_DispatchThreadID)
{
    ...
    if (any(dispatchID >= dstSize))
        return;
    
    float2 srcSize = u_Params.src1 - u_Params.src0;
    float2 scale = srcSize / dstSize;
    
    float2 t = float2(dispatchID) * scale / srcSize;
    float2 uv = lerp(u_Params.src0, u_Params.src1, t) / TextureSize(u_Input);
    
    float4 color = u_Input.SampleLevel(u_Sampler, uv, 0);
    o_Output[dispatchID + u_Params.dst0] = color;
}
